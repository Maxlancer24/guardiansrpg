...
function begin(){current=queue.shift();t=0;eventBeat=0;if(!current){display=clone(result.next);ready=!result.finished;status('');choice=null;intent=battle.chooseEnemy();if(result.finished){finishedAt=clock;$('result').hidden=false;$('result').textContent=result.winner===0?tr('VICTORIA','VICTORY'):tr('DERROTA','DEFEAT');status($('result').textContent)}ui();return}current.duration=eventDuration(current);current.beats=impacts(current);if(current.type==='attack'&&current.actor===0&&key==='jessie'){current.variant=variant;variant=1-variant}if(current.type==='protection'){display.actors[0].guard=true;guardStarted[0]=clock}if(current.type==='zoe-special')sound('parry');status((current.actor===null?'':name(current.actor)+' · ')+(['cinematic','zoe-special','protection'].includes(current.type)?heroes[key].skill:current.type==='counter'?tr('Contraataque','Counterattack'):actions[current.type.toUpperCase()]||tr('Resolviendo','Resolving')));ui()}
...
function render(){if(!display)return;const camera=frameCamera();ctx.clearRect(0,0,1280,camera.h);
 ...
 ctx.save();ctx.translate(camera.x,camera.y);ctx.scale(camera.zoom,camera.zoom);const ps=positions();specialBackdrop(ps);if(current?.type==='zoe-special'&&$('effects').checked){ctx.fillStyle='rgba(2,9,18,'+(.65*ease(t/300)*(1-ease((t-3700)/600)))+')';ctx.fillRect(0,0,1280,650)}
