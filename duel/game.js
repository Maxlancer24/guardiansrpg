...
function begin(){current=queue.shift();t=0;eventBeat=0;if(!current){display=clone(result.next);ready=!result.finished;status('');choice=null;intent=battle.chooseEnemy();if(result.finished){window.DemoAnalytics?.finish(result.winner);finishedAt=clock;$('result').hidden=false;$('result').textContent=result.winner===0?tr('VICTORIA','VICTORY'):tr('DERROTA','DEFEAT');status($('result').textContent)}ui();return}current.duration=eventDuration(current);current.beats=impacts(current);if(current.type==='attack'&&current.actor===0&&key==='jessie'){current.variant=variant;variant=1-variant}if(current.type==='protection'){display.actors[0].guard=true;guardStarted[0]=clock}if(current.type==='zoe-special')sound('parry');status((current.actor===null?'':name(current.actor)+' · ')+(['cinematic','zoe-special','protection'].includes(current.type)?heroes[key].skill:current.type==='counter'?tr('Contraataque','Counterattack'):actions[current.type.toUpperCase()]||tr('Resolviendo','Resolving')));ui()}
function submit(){if(!ready||paused||!choice||(choice==='SPECIAL'&&display.actors[0].used))return;try{window.DemoAnalytics?.start('1v1',[key]);result=battle.resolve(choice,intent);guardStarted={};result.actions.forEach((a,i)=>{if(a==='DEFEND')guardStarted[i]=clock});ready=false;queue=combine(result.events);ui();begin()}catch(e){status(tr('No se pudo resolver la acción. Reinicia el duelo.','Could not resolve action. Restart the duel.'));console.error(e)}}
...
function txt(s,x,y,size=18,color='#fff0d7',align='center'){ctx.fillStyle=color;ctx.font='600 '+size+'px Georgia';ctx.textAlign=align;ctx.fillText(s,x,y)}
 ...
 // Screen-space effects must not inherit the mobile character camera.
 function fillViewport(top=0,height=canvas.height){ctx.save();ctx.setTransform(1,0,0,1,0,0);ctx.fillRect(0,top,canvas.width,height);ctx.restore();}
 function specialBackdrop(ps){if(!cinematicEnabled())return;const st=sceneTime(),fade=ease(st/400)*(1-ease((st-3750)/550));
  ctx.fillStyle='rgba(2,7,17,'+.72*fade+')';fillViewport();ctx.fillStyle='rgba(2,5,11,'+.92*fade+')';fillViewport(0,40);fillViewport(canvas.height-34,34);
 ...
 function specialEffects(ps){if(!cinematicEnabled())return;const st=sceneTime(),p=ps[0],charge=ease((st-2700)/580)*(1-ease((st-3350)/240));
  ...
  ctx.fillStyle='rgba(255,232,188,'+wash+')';fillViewport();
...
function cutin(){if(!current||!['protection','cinematic','zoe-special'].includes(current.type))return;const ct=80+(t-80)/2;if(t<80||ct>870)return;const alpha=ease((ct-80)/130)*(1-ease((ct-670)/200)),slide=-90*(1-ease((ct-80)/160)),y=130,h=285;ctx.save();ctx.globalAlpha=alpha;ctx.fillStyle='#071423f5';fillViewport();ctx.strokeStyle='#e7bb68';ctx.lineWidth=2;ctx.strokeRect(0,y,1280,h);ctx.save();ctx.beginPath();ctx.rect(0,y,720,h);ctx.clip();const im=imgs[key==='zoe'?'zcutin':key==='garrick'?'gcutin':'jcutin'],scale=798.72/im.width;ctx.drawImage(im,50+slide,65,im.width*scale,im.height*scale);ctx.restore();txt(heroes[key].name.toUpperCase(),940,y+115,42,'#ffe1a3');txt(heroes[key].skill,940,y+164,23,'#98f4df');ctx.restore()}
function frameCamera(){
 const rect=canvas.getBoundingClientRect?.()||{width:1280,height:650};
 const width=Math.max(1,rect.width),height=Math.max(1,rect.height),h=Math.round(1280*height/width);
 if(canvas.width!==1280)canvas.width=1280;if(canvas.height!==h)canvas.height=h;
 const mobile=width<=700,zoom=mobile?1.5:1,ground=h*(mobile?.64:.68),x=640-640*zoom,y=ground-460*zoom;
 // Hit areas use exactly the same camera as the sprites, including rotation/resizing.
 for(const [id,node]of [[0,$('select-actor')],[1,$('select-target')]]){
  const p=homes[id];node.style.left=((p.x-95)*zoom+x)/1280*100+'%';
  node.style.top=((p.y-270)*zoom+y)/h*100+'%';
  node.style.width=190*zoom/1280*100+'%';node.style.height=285*zoom/h*100+'%';
 }
 return {h,zoom,x,y,ground};
}
function render(){if(!display)return;const camera=frameCamera();ctx.clearRect(0,0,1280,camera.h);
 const bg=imgs.arena,scale=Math.max(1280/bg.width,camera.h/bg.height);
 const by=Math.max(camera.h-bg.height*scale,Math.min(0,camera.ground-bg.height*.70*scale));
 ctx.drawImage(bg,(1280-bg.width*scale)/2,by,bg.width*scale,bg.height*scale);
 ctx.fillStyle='#03100c33';ctx.fillRect(0,0,1280,camera.h);
 ctx.save();ctx.translate(camera.x,camera.y);ctx.scale(camera.zoom,camera.zoom);const ps=positions();specialBackdrop(ps);if(current?.type==='protection'&&$('effects').checked){ctx.fillStyle='rgba(2,9,18,'+(.55*ease(t/300)*(1-ease((t-2300)/600)))+')';fillViewport();}if(current?.type==='zoe-special'&&$('effects').checked){ctx.fillStyle='rgba(2,9,18,'+(.65*ease(t/300)*(1-ease((t-3700)/600)))+')';fillViewport()}
...
for(const l of labels){const k=clamp((clock-l.at)/1300),p=ps[l.id],lane=l.lane||0;ctx.save();ctx.globalAlpha=1-k;txt(String(l.value),p.x+[-38,38,-80,80,0][lane%5],p.y-250-40*Math.floor(lane/2)-50*k,22,l.color);ctx.restore()}ctx.restore();ctx.save();ctx.translate(0,(camera.h-650)/2);cutin();ctx.restore()}
