...
let key='zoe',battle,display,result,current=null,queue=[],t=0,clock=0,last=0,ready=false,active=false,paused=false,loading=false,choice='ATTACK',intent='ATTACK',labels=[],bursts=[],struck={},deadAt={},guardStarted={},eventBeat=0,variant=0,rafStarted=false,serial=0,finishedAt=null;
...
function ui(){
 ...
 for(const a of display.actors){
  ...
  card.innerHTML='<h2>'+name(a.id)+'</h2><progress max="'+a.max+'" value="'+a.hp+'" aria-label="'+name(a.id)+' HP"></progress><p>'+a.hp+' / '+a.max+' HP</p><p>STR '+a.str+' · AGI '+a.agi+' · CON '+a.con+'</p><p class="state">'+state+' · '+(a.rank===1?tr('Actúa primero','Acts first'):tr('Actúa después','Acts second'))+'</p>';
  $('cards').append(card);
 }
 ...
 for(const [v,label]of Object.entries(actions)){
  const b=button(label,()=>{choice=v;submit()});b.disabled=!ready||paused||(v==='SPECIAL'&&display.actors[0].used);
  b.className=v==='ATTACK'?'primary':'';b.id='action-'+v.toLowerCase();
...
function begin(){current=queue.shift();t=0;eventBeat=0;if(!current){display=clone(result.next);ready=!result.finished;intent=battle.chooseEnemy();if(result.finished){finishedAt=clock;$('result').hidden=false;$('result').textContent=result.winner===0?tr('VICTORIA','VICTORY'):tr('DERROTA','DEFEAT');status($('result').textContent)}ui();return}current.duration=eventDuration(current);current.beats=impacts(current);if(current.type==='attack'&&current.actor===0&&key==='jessie'){current.variant=variant;variant=1-variant}if(current.type==='protection'){display.actors[0].guard=true;guardStarted[0]=clock}if(current.type==='zoe-special')sound('parry');status((current.actor===null?'':name(current.actor)+' · ')+(['cinematic','zoe-special','protection'].includes(current.type)?heroes[key].skill:current.type==='counter'?tr('Contraataque','Counterattack'):actions[current.type.toUpperCase()]||tr('Resolviendo','Resolving')));ui()}
function submit(){if(!ready||paused)return;try{result=battle.resolve(choice,intent);guardStarted={};result.actions.forEach((a,i)=>{if(a==='DEFEND')guardStarted[i]=clock});ready=false;queue=combine(result.events);ui();begin()}catch(e){status(tr('No se pudo resolver la acción. Reinicia el duelo.','Could not resolve action. Restart the duel.'));console.error(e)}}
...
$('replay').onclick=()=>{paused=false;reset()};$('give-feedback').onclick=()=>{$('feedback-panel').hidden=false;$('feedback-text').focus()};$('dismiss-guide').onclick=()=>{$('first-turn-guide').hidden=true};$('reset').onclick=()=>{if(active)reset()};$('change-hero').onclick=changeHero;$('pause').onclick=()=>{paused=!paused;if(paused)stopSounds();ui()};$('sound').onchange=()=>{if(!$('sound').checked)stopSounds()};
