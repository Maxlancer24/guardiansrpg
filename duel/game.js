...
function render(){if(!display)return;ctx.clearRect(0,0,1280,650);/* The stage owns the single continuous backdrop; the actor canvas stays transparent. */const ps=positions();specialBackdrop(ps);if(current?.type==='zoe-special'&&$('effects').checked){ctx.fillStyle='rgba(2,9,18,'+(.65*ease(t/300)*(1-ease((t-3700)/600)))+')';ctx.fillRect(0,0,1280,650)}
